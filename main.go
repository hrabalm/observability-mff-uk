...
func init() {
	...
	// Logging: Enable debug level of logging. Uncomment following line.
	logrus.SetLevel(logrus.DebugLevel)
	// Set JSON formatter.
	// Logging: Enable json formatting for logging. Uncomment following line.
	logrus.SetFormatter(&logrus.JSONFormatter{})
	// Set logging to a file. Comment out following 2 lines to log on the console.
	// Logging: Enable logging to a file. Uncomment following 2 lines.
	f := getLogFile()
	logrus.SetOutput(f)
	// Register Prometheus metrics
	// Metrics: Register metrics. Uncomment following 3 lines.
	prometheus.Register(totalRequests)
	prometheus.Register(responseStatus)
	prometheus.Register(httpDuration)
	// Set tracing provider
	// Tracing: Configure tracing provider. Uncomment following 4 lines.
	tp, err := tracerProvider(tracingUrl)
	if err != nil {
		log.Fatal(err)
	}
	// Tracing: Enable tracing provider. Uncomment following line.
	otel.SetTracerProvider(tp)
	// Tracing: Enable cross-boundary context propagation for tracing. Uncomment following line.
...
func main() {
	...
	// Metrics: Expose metrics endpoint. Uncomment following line.
	r.Handle("/metrics", promhttp.Handler())
	// Tracing: Enable tracing middleware. Uncomment following line.
	r.Use(tracingMiddleware)
	// Metrics: Enable metrics middleware. Uncomment following line.
	r.Use(metricsMiddleware)
	// Logging: Enable logging middleware. Uncomment following line.
